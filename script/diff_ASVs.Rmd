---
title: "Differential ASVs analysis"
author: "xiaolin wang"
date: '2019-10-30'
...
##Differential ASVs/OTUs between Col-0 and individual mutant (mfec, fec, min7, cad1)

###This is an R Markdown document to dectect differential ASVs/OTUs between Col-0 and individual mutant (mfec,fec, min7, cad1). 

###The mfec part and the cad1 part were analized respectively due to the experimental progress
...
##(2) Differentially abundant ASVs/OTUs were detected using EdgeR generalized linear model (GLM) approach
...
###(2.1) Natural_total GLM: negative binomial generalized log-linear model
...
natural_total_glmLRT_otu <- cbind(Col0 = rowMeans(dgelist_natural_total$counts[,dgelist_natural_total$samples$group == "Col0"])/1000, 
                                  ...
                                  as.data.frame(topTags(lrt_natural_total, n = dim(dgelist_natural_total$counts)[1],adjust.method = "fdr")))
...
###(2.2) Natural_endo GLM: negative binomial generalized log-linear model
...
###(2.3) Syncom_endo_mfec GLM: negative binomial generalized log-linear model
...
###(2.4) Syncom_endo_fec GLM: negative binomial generalized log-linear model
...
###(2.5) Syncom_endo_min7 GLM: negative binomial generalized log-linear model
...
group_cad1_syncom_endo <- as.character(cad1_syncom_endo$genotype) 
group_cad1_syncom_endo
```

###(2) Differentially abundant OTUs were detected using EdgeR generalized linear model (GLM) approach
...
###Collect all the informations and save the data
###Relative abundance, GLM stats, taxonomy, fasta
cad1_syncom_endo_glmLRT_otu <- cbind(Col0 = rowMeans(dgelist_cad1_syncom_endo$counts[,dgelist_cad1_syncom_endo$samples$group == "Col0"])/1000, 
                                     cad1 = rowMeans(dgelist_cad1_syncom_endo$counts[,dgelist_cad1_syncom_endo$samples$group == "cad1"])/1000,
...
cad1_syncom_endo_glmLRT_otu_tax_fasta$sig <- as.factor(ifelse(cad1_syncom_endo_glmLRT_otu_tax_fasta$FDR >= 0.05,"NS",
                                                              ifelse(cad1_syncom_endo_glmLRT_otu_tax_fasta$logFC > 0,"Enriched","Depleted")))
...
##Part3, Optimized Visualization using evolview, the pics are provided in the website below:

###Natural endo mfec enriched or depleted ASVs was provided in below link:
(https://evolgenius.info//evolview-v2/#shared/BsDqipvNwe/1421)

###Syncom endo mfec enriched or depleted ASVs was provided in below link:
(https://evolgenius.info//evolview-v2/#shared/YCTPTFPHXg/1422)

###Syncom endo cad1 enriched or depleted ASVs was provided in below link:
(https://evolgenius.info//evolview-v2/#shared/hvAPiIZCvu/1423)
